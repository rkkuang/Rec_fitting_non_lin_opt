...
[~,minx_index1] = min(x);
sortx = sort(x);
if abs(sortx(1)-sortx(2))<=eps
minx2 = min(x(x>=x(minx_index1)));
[~,minx_index2] = find(x == minx2);
minx_index2 = minx_index2(end);%190410
else
minx2 = min(x(x>x(minx_index1)));
[~,minx_index2] = find(x == minx2);
minx_index2 = minx_index2(end);%190410
...
%[~,max_index] = find(x>=(max(x(minx_index1),x(minx_index2)))+eps); %190416 wrong
% X:[69,149,149,233] Y[191,47,237,93]
max_index = [];
for i=1:4
   if i~= minx_index1 && i~= minx_index2
       max_index(end+1)=i;
   end
end
